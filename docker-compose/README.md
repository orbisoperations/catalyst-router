...
3. **Node Token Minting**: On startup, the orchestrator uses the system token to mint a NODE token for itself (principal: `CATALYST::NODE`)
...
2. Orchestrator calls auth service `permissions(callerToken)` to get a permissions handler
3. Auth service validates the token and evaluates Cedar policies for the requested action
