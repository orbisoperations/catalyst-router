...
export function createVideoSubscribe(deps: VideoSubscribeDeps) {
  const { getCatalog, auth, config, nodeId, domains = [], relayManager, tracer } = deps
  ...
  handler.post('/subscribe/:streamName{.+}', async (c) => {
    ...
    try {
      ...
      logger.error`Auth service error for ${streamName} (${Date.now() - authStart}ms): ${err}`
      return c.json({ success: false, error: 'Forbidden' }, 403)
    }

    const authLatencyMs = Date.now() - authStart
    logger.info`Auth evaluated for ${streamName}: allowed=${result.success && result.allowed} (${authLatencyMs}ms)`
