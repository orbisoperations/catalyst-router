...
program
  ...
  .addOption(new Option('-o, --output <path>', 'Output YAML file path').default(DEFAULTS.output))
  .addOption(new Option('-m, --mode <mode>', 'Deployment mode').choices(['native', 'docker']).default(DEFAULTS.mode))
  .addOption(new Option('--dry-run', 'Print YAML to stdout instead of writing a file'))
  ...
  .addOption(new Option('--peering-secret <secret>', 'iBGP peering secret').env('CATALYST_PEERING_SECRET'))
  ...
  .addOption(new Option('--bootstrap-token <token>', 'Auth bootstrap token').env('CATALYST_BOOTSTRAP_TOKEN'))
  .addOption(new Option('--log-level <level>', 'Log level').choices(['debug', 'info', 'warn', 'error']).default(DEFAULTS.logLevel))
  ...
  .addOption(new Option('--otel-version <ver>', 'OTEL Collector version').default(DEFAULTS.otelVersion))
  ...
  .addOption(new Option('--boot-part-size <size>', 'Boot partition size').default(DEFAULTS.bootPartSize))
  .addOption(new Option('--root-part-size <size>', 'Root partition size'))

  .addOption(new Option('--rpi-image-gen <path>', 'Path to rpi-image-gen').env('RPI_IMAGE_GEN_PATH'))
  .addOption(new Option('--skip-validation', 'Skip layer validation'))
...
function resolveRpiImageGen(explicit?: string): string | undefined {
  if (explicit) {
    return existsSync(explicit) ? resolve(explicit) : undefined
  }
  // Auto-detect common locations
  const candidates = ['../rpi-image-gen', '../../rpi-image-gen']
...
function resolveSrcDir(opts: { output: string }): string {
  // Source dir is the builds/rpi directory (parent of config/)
  return resolve(dirname(opts.output), '..')
}
...
program.action(async (opts) => {
  ...
  if (!resolved.skipValidation) {
    const rpiPath = resolveRpiImageGen(resolved.rpiImageGen)
    const srcDir = resolveSrcDir(resolved)
    if (rpiPath) {
      const results = validateLayers(config, rpiPath, srcDir)
      printValidationResults(results)
      const failures = results.filter((r: { found: boolean }) => !r.found)
      if (failures.length > 0) {
        process.stderr.write(`\n\u2717 Validation failed: ${failures.length} layer(s) not found.\n`)
        process.stderr.write(`  Missing: ${failures.map((f: { name: string }) => f.name).join(', ')}\n\n`)
        process.exit(1)
      }
    }
  }
  ...
  if (resolved.dryRun) {
    ...
    printInstructions(resolved.output, resolved, process.stderr)
  } else {
    const absPath = resolve(resolved.output)
    writeFileSync(absPath, yamlContent, 'utf-8')
    printInstructions(absPath, resolved, process.stderr)
  }
})
