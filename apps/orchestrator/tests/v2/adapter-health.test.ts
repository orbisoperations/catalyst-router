...
describe('AdapterHealthChecker', () => {
  ...
  // -------------------------------------------------------------------------
  // 4. Non-HTTP protocol → 'down', no fetch made
  // -------------------------------------------------------------------------
  it('non-HTTP protocol → down, no fetch made', async () => {
    ...
    expect(checker.getHealth('tcp-svc')?.healthStatus).toBe('down')
  ...
  // -------------------------------------------------------------------------
  // 6. Missing endpoint → 'down', no fetch made
  // -------------------------------------------------------------------------
  it('missing endpoint → down, no fetch made', async () => {
    const mockFetch = vi.fn()
    vi.stubGlobal('fetch', mockFetch)
    ...
    expect(checker.getHealth('no-ep')?.healthStatus).toBe('down')
    expect(mockFetch).not.toHaveBeenCalled()
  })
  ...
  // -------------------------------------------------------------------------
  // Additional: non-OK status without prior 'up' → 'down'
  // -------------------------------------------------------------------------
  it('non-OK status without prior up → down', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(makeMockResponse(500, false)))

    const routes = [makeRoute('svc')]
    await checker.checkAll(routes)

    expect(checker.getHealth('svc')?.healthStatus).toBe('down')
