...
describe('Peering E2E Lifecycle (WebSocket Transport)', () => {
  ...
  it('should connect Peer A to Peer B and sync existing routes', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (routes.some((r: LocalRoute) => r.service.name === 'pre-existing-on-a-ws')) {
        onA = true
        break
      }
    ...
    for (let i = 0; i < 60; i++) {
      ...
      try {
        ...
        if (lastRoutesB.some((r: LocalRoute) => r.service.name === 'pre-existing-on-a-ws')) {
          synced = true
          break
        }
      } catch {
  ...
  it('should propagate services bidirectionally', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (routes.some((r: LocalRoute) => r.service.name === 'service-on-a-ws')) {
          propagatedToB = true
          break
        }
      } catch {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (routes.some((r: LocalRoute) => r.service.name === 'service-on-b-ws')) {
          propagatedToA = true
          break
        }
      } catch {
  ...
  it('should disconnect and cleanup routes', async () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      try {
        ...
        if (!lastRoutesB.some((r: LocalRoute) => r.service.name === 'service-on-a-ws')) {
          cleanedOnB = true
          break
        }
      } catch {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      try {
        ...
        if (!lastRoutesA.some((r: LocalRoute) => r.service.name === 'service-on-b-ws')) {
          cleanedOnA = true
          break
        }
      } catch {
