...
describe.skipIf(skipTests)('Peering E2E Lifecycle (Containerized)', () => {
  ...
  it('should connect Peer A to Peer B and sync existing routes', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (routes.some((r: LocalRoute) => r.service.name === 'pre-existing-on-a')) {
        onA = true
        break
      }
    ...
    for (let i = 0; i < 60; i++) {
      ...
      try {
        ...
        if (peers.some((p: AuthorizedPeer) => p.id === 'peer-b')) {
          connected = true
          break
        }
      } catch {
    ...
    let synced = false
    let lastRoutesB: LocalRoute[] = []
    for (let i = 0; i < 60; i++) {
      // Extreme iterations (60s)
      await new Promise((r) => setTimeout(r, 1000))
      try {
        ...
        if (lastRoutesB.some((r: LocalRoute) => r.service.name === 'pre-existing-on-a')) {
          synced = true
          break
        }
      } catch {
  ...
  it('should propagate services bidirectionally', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (routes.some((r: LocalRoute) => r.service.name === 'service-on-a')) {
          propagatedToB = true
          break
        }
      } catch {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (routes.some((r: LocalRoute) => r.service.name === 'service-on-b')) {
          propagatedToA = true
          break
        }
      } catch {
  ...
  it('should disconnect and cleanup routes', async () => {
    ...
    try {
      ...
      const peerBRecord = peers.find((p: AuthorizedPeer) => p.id === 'peer-b')
    ...
    for (let i = 0; i < 15; i++) {
      ...
      try {
        ...
        if (!lastRoutesB.some((r: LocalRoute) => r.service.name === 'service-on-a')) {
          cleanedOnB = true
          break
        }
      } catch {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      try {
        ...
        if (!lastRoutesA.some((r: LocalRoute) => r.service.name === 'service-on-b')) {
          cleanedOnA = true
          break
        }
      } catch {
