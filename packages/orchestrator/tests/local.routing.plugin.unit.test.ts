...
describe('LocalRoutingTablePlugin Comprehensive Tests', () => {
  ...
  const createCtx = (
    resource: 'localRoute',
    resourceAction: 'create' | 'update' | 'delete',
    data: unknown
  ): PluginContext => ({
    action: { resource, resourceAction, data } as unknown as Action,
    state,
    results: [],
  ...
  describe('Update Actions', () => {
    it('should update an existing internal route', async () => {
      ...
      const route = ctx.state
        .getInternalRoutes()
        .find((r) => r.service.name === 'internal-service-update')
    ...
    it('should update an existing proxy route', async () => {
      ...
      const route = ctx.state
        .getProxiedRoutes()
        .find((r) => r.service.name === 'proxy-service-update')
