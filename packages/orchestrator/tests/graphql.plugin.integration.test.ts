...
import type { PluginContext, AuthContext } from '../src/plugins/types.js'
...
describe('GraphQL Plugin E2E with Containers', () => {
  ...
  beforeAll(async () => {
    ...
    booksContainer = await new GenericContainer('books-service:test')
      ...
      .withWaitStrategy(
        Wait.forLogMessage(/running|listening|starting/i).withStartupTimeout(180_000)
      )
      .start()
  ...
  it('should handle full lifecycle: unconfigured -> add -> update -> delete', async () => {
    ...
    const context: PluginContext = {
      ...
      state,
      authxContext: {} as unknown as AuthContext,
    }
    ...
    const queryGateway = async (query: string) => {
      ...
      return {
        ...
        json: json as {
          data?: {
            listLocalRoutes?: unknown[]
            listPeers?: unknown[]
            applyAction?: { id: string }
            deletePeer?: { success: boolean }
            books?: unknown[]
            movies?: unknown[]
