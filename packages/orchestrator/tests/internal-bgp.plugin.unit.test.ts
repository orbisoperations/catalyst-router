...
const mockFactory = () => mockSession as unknown as any
...
describe('InternalBGPPlugin Unit Tests', () => {
  ...
  it('should trigger broadcast when a local route is updated', async () => {
    ...
    const spyFactory = (_endpoint: string) => {
      callCount++
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return mockSession as unknown as any
    }
  ...
  it('should propagate learned routes to other peers and await results (transit)', async () => {
    ...
    const transitPeerSession = {
      ...
      open: async () => ({
        success: true,
        peerInfo: { id: 'peer-c', as: 300, endpoint: 'http://peer-c:3000/rpc', domains: [] },
      }),
    }

    const factory = (endpoint: string) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (endpoint === 'http://peer-c:3000/rpc') return transitPeerSession as unknown as any
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return mockSession as unknown as any
    }
  ...
  it('should fail if propagation to transit peer fails', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const factory = (_endpoint: string) => failingSession as unknown as any
    const plugin = new InternalBGPPlugin(factory)
