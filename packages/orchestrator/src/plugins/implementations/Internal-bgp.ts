...
import type { ApplyActionResult, ServiceDefinition } from '../../rpc/schema/index.js'
import { getConfig } from '../../config.js'
import { getHttpPeerSession } from '../../rpc/client.js'

import type { PeerInfo, AuthorizedPeer } from '../../rpc/schema/peering.js'
import {
...
export class InternalBGPPlugin extends BasePlugin {
  ...
  private async handleConfigCreatePeer(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      const peerData = (
        openRes as { peerInfo: { id: string; as: number; endpoint: string; domains: string[] } }
      ).peerInfo
  ...
  private async handleConfigUpdatePeer(context: PluginContext): Promise<PluginResult> {
    ...
    try {
      ...
      return {
        ...
        error: {
          pluginName: this.name,
          message: `Failed to handshake with new endpoint: ${message}`,
        },
      }
    }
  ...
  private async handleProtocolOpen(context: PluginContext): Promise<PluginResult> {
    ...
    if (existingByEndpoint && existingByEndpoint.id !== peerInfo.id) {
      console.log(
        `[InternalAS] Unifying peer ID for ${peerInfo.endpoint}: ${existingByEndpoint.id} -> ${peerInfo.id}`
      )
  ...
  private async handleProtocolUpdate(context: PluginContext): Promise<PluginResult> {
    ...
    console.log(
      `[InternalAS] Handling BGP UPDATE session from ${sourcePeerId} with ${updateMessages.length} messages`
    )
    ...
    for (const msg of updateMessages) {
      if (msg.type === 'add') {
        ...
        if (asPath.includes(myAs)) {
          console.warn(
            `[InternalAS] Loop detected for route ${(msg.route as { name: string }).name} (Path: ${asPath}). Dropping.`
          )
          continue
        }
    ...
    if (routesToPropagate.length > 0) {
      ...
      console.log(
        `[InternalAS] Propagating ${routesToPropagate.length} routes to ${peers.length} peers.`
      )
      ...
      for (const peer of peers) {
        for (const item of routesToPropagate) {
          ...
          propagationPromises.push(
            this.sendIndividualUpdate(context, peer.id, {
              type: 'add',
              route: item.route,
              asPath: newPath,
            } as UpdateMessage)
  ...
  private async executeHandshakeAndSync(
  ...
  private async syncExistingRoutesToPeer(context: PluginContext, peerId: string): Promise<void> {
    ...
    console.log(
      `[InternalAS] syncExistingRoutesToPeer: Peer=${peerId}, RouteCount=${context.state.getAllRoutes().length}`
    )
  ...
  private async sendIndividualUpdate(
    context: PluginContext,
    peerId: string,
    updateMsg: UpdateMessage
  ): Promise<ApplyActionResult> {
    const peer = context.state.getPeer(peerId)
    if (!peer || !peer.endpoint || peer.endpoint === 'unknown')
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : String(e)
      console.error(
        `[InternalAS] Failed to send update to peer ${peerId} at ${peer.endpoint}:`,
        message
      )
  ...
  private async broadcastRouteUpdate(context: PluginContext): Promise<PluginResult> {
    ...
    if (action.resourceAction === 'create') {
      ...
      if (!result.success) {
        console.error(
          `[InternalAS] Failed to parse localRoute create action:`,
      ...
      if (!result.success) {
        console.error(
          `[InternalAS] Failed to parse localRoute update action:`,
          result.error.message
        )
        return { success: true, ctx: context }
      }
      ...
      if (!result.success) {
        console.error(
          `[InternalAS] Failed to parse localRoute delete action:`,
          result.error.message
        )
        return { success: true, ctx: context }
      }
    ...
    console.log(
      `[InternalAS] Broadcasting ${updateType} to ${peers.length} peers via WebSocket RPC.`
    )
