...
import { createRouteTable, RouteTable } from '../state/route-table.js';
...
import { IBGPConfigHandler } from '../plugins/implementations/ibgp-config-handler.js';
import { IBGPProtocolHandler } from '../plugins/implementations/ibgp-protocol-handler.js';
import { RouteBroadcaster, StateProvider } from '../plugins/implementations/route-broadcaster.js';
import { IBGPProtocolResource, IBGPProtocolResourceAction, ListPeersResult, PeerInfo, UpdateMessage, IBGPScope, IBGPConfigResource, IBGPConfigResourceAction } from './schema/peering.js';
...
export class OrchestratorRpcServer extends RpcTarget {
    ...
    private gatewayPlugin?: GatewayIntegrationPlugin;

    constructor(config?: OrchestratorConfig) {
        super();
        this.config = config || getConfig();

        // Initialize State with a fresh RouteTable instance for isolation
        this.state = createRouteTable();
        ...
        // Conditionally add Gateway Plugin (event-driven mode)
        // Subscribes to ALL route events (local + peer) to enable cross-node gateway federation
        // Maintains internal service list, updates incrementally from events
        if (this.config.gqlGatewayConfig) {
            this.gatewayPlugin = new GatewayIntegrationPlugin(this.config.gqlGatewayConfig);
            this.gatewayPlugin.start();
            plugins.push(this.gatewayPlugin);
        }
        ...
        // Split iBGP into focused handlers (formerly InternalBGPPlugin)
        const ibgpConfigHandler = new IBGPConfigHandler(sessionFactory);
        const ibgpProtocolHandler = new IBGPProtocolHandler(sessionFactory);

        // RouteBroadcaster is NOT a plugin - it subscribes to events instead
        // This decouples it from LocalRouting's action schema
        const stateProvider: StateProvider = () => this.state;
        this.routeBroadcaster = new RouteBroadcaster(stateProvider, sessionFactory);
        this.routeBroadcaster.start();
     ...
     */
    private isSecretValid(providedSecret: string): boolean {
        const expected = this.config.ibgp.secret;
    ...
    async applyAction(action: Action): Promise<ApplyActionResult> {
        // Validate action at RPC boundary
        ...
        if (!validation.success) {
            ...
            return {
                success: false,
                results: [],
                error: `Invalid action: ${errorDetails}`
            };
        }
