...
*   **New**: Organizations are the administrative domain.
    ...
    *   *Gap*: We need to decide if we hardcode the same 3 roles or allow flexible RBAC. (Recommendation: Flexible).
...
*   **New**: The Node allows organizations to build a **Service Mesh / API Gateway**.
    ...
    *   *Parity*: We must implement a "Service Killswitch" to match the `accessSwitch` capability.
...
*   **New**: "Partnerships" allow another Org to access their services.
    ...
    *   *Parity*: Our Peering state machine (`Idle`, `Connect`, `Active`) closely mirrors the Invite/Active flow.
...
*   **New**:
    ...
    *   *Parity*: We need an explicit "Key Rotation" command in the CLI (Phase 3/6).
...
1.  **Schema Certification / Registry**:
    *   *Legacy*: `data-channel-certifier` performs SDL Federation validation before a service is active.
    *   *New Plan*: Implicitly handled by the Gateway/Mesh, but no explicit "Certification" step or Registry exists.
    *   *Recommendation*: Add a `verification` step in **Phase 2** to validate Peer Schemas.

2.  **Compliance Auditing**:
    *   *Legacy*: Audit logs for permission/role changes in `authx` APIs.
    *   *New Plan*: **Phase 9 (Observability)** covers metrics, but **Audit Logging** (who changed what policy) is missing.
    *   *Recommendation*: Add structured Audit Logging to **Phase 7 (Policy Management)**.

3.  **Endpoint Validation**:
    *   *Legacy*: `data-channel-certifier` validates the reachability and health of the endpoint (URL) alongside the schema.
    *   *New Plan*: **Phase 2** covers routing, but active health checking/validation of specific service endpoints is not explicitly detailed.
    *   *Recommendation*: Integrate **Active Health Checks** (Envoy) and startup validation in **Phase 2**.
