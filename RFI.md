...
*   **Deployment Model**: **Config-Driven Automation (GitOps)**.
    *   *Decision*: We will NOT build a "Click-to-Spin-Up" dynamic UI for now.
    *   *Rationale*: Business model supports scheduled/config-based updates. JIT provisioning is unnecessary complexity.
*   **Requirements**:
    ...
    *   **Compute**: Decide between Cloud Run (Serverless) vs GKE (Kubernetes) vs GCE (VMs).
        *   *Note*: Cloud Run is eager to scale to zero, which might conflict with our BGP-style long-lived connections.
