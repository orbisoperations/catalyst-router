...
- **Cedar Schema** (`packages/authorization/src/policy/src/definitions/schema.cedarschema`): Defines 5 principal types, 5 resource types, and 14 actions under the `CATALYST` namespace.
- **Cedar Policies** (5 `.cedar` files): Per-principal permit policies with `trustedDomains`/`trustedNodes` constraints.
...
1. **Schema file mismatch**: Code imports `schema.cedarschema` but only `schema.cedarschema` exists on disk.
...
| Schema file naming consistency                           | Must     | Fix `schema.cedarschema` vs `schema.cedarschema` mismatch |
...
| Action                | ADMIN | NODE | NODE_CUSTODIAN | DATA_CUSTODIAN | USER |
| --------------------- | ----- | ---- | -------------- | -------------- | ---- |
| LOGIN                 | x     |      |                |                | x    |
| MANAGE                | x     |      |                |                |      |
| IBGP_CONNECT          | x     | x    | x              |                |      |
...
- [Cedar Schema Definition](../../packages/authorization/src/policy/src/definitions/schema.cedarschema)
